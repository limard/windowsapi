...
func LaunchInActiveSesstion(cmd string) (pid int, handle uintptr, err error) {
	...
	log.Println("SesstionId", sesstionId)
	...
	// 	log.Println("OpenProcessToken", err)
	// 	goto Exit
	// }
	// defer hToken.Close()
	...
	if err != nil {
		log.Println("WTSQueryUserToken:", err)
		goto Exit
	}
	defer hToken.Close()
	...
	if err != nil {
		log.Println("EnableAllPrivileges:", err)
		// goto Exit
	}
	...
	if err != nil {
		log.Println("DuplicateTokenEx:", err)
		goto Exit
	}
	...
	if err != nil {
		log.Println("SetTokenInformation:", err)
		goto Exit
	}
	...
	if err != nil {
		log.Println("CreateEnvironmentBlock:", err)
		goto Exit
	}
	...
	if err != nil {
		log.Println("CreateProcessAsUser:", err)
		goto Exit
	}
...
// CreateUserProcess
func LaunchInXSession(cmd string, creationFlags uint32, sessionId uint32) error {
	// hProc = OpenProcess(PROCESS_ALL_ACCESS, FALSE, GetCurrentProcessId());
	//curPid := syscall.Getpid()
	//log.Printf("pid: %d", curPid)
	//hProc, err := syscall.OpenProcess(PROCESS_ALL_ACCESS, false, uint32(curPid))
	//if err != nil {
	//	log.Printf("OpenProcess: %v", err)
	//	return err
	//}
